...
def make_tweet_fn(text, time, lat, lon):
    ...
    "*** YOUR CODE HERE ***"
    tweet_dict = {'text' : text, 'time': time, 'lat':lat, 'lon':lon}
    return lambda dict_element: tweet_dict[dict_element]
...
def extract_words(text):
    ...
    """
    "*** YOUR CODE HERE ***"
    replaced_letters = lambda letter_string: letter_string if letter_string.isalpha() else ' '
    raw_list = text.split()
    final= []
    for clean_list in raw_list:
        splitted_letter = (''.join( [ replaced_letters(clean_list[index]) for index in range(0, len(clean_list)) ])).split()
        for new_word in splitted_letter:
...
def analyze_tweet_sentiment(tweet):
    ...
    False
    """
    ave = make_sentiment(None)
    sentiment_list = []
    words_in = tweet_words(tweet)
    total, counter = 0,0
    for each_word in words_in: #Find total sentiment and number of words
        if has_sentiment(get_word_sentiment(each_word)) == True:
            total += sentiment_value(get_word_sentiment(each_word))
            counter += 1
    if counter == 0: #The sentiment is neutral, or None
        return make_sentiment(None)
    else: #return average
        ave = make_sentiment(total/counter)
        return ave
...
def find_centroid(polygon):
    ...
    """
    "*** YOUR CODE HERE ***"
    #traverses through the points and uses the centroid formula to find the area and points
    area = 0 
    centroid_x = 0
    centroid_y = 0
    for i in range(len(polygon)-1):
        x,y = latitude(polygon[i]), longitude(polygon[i])
        next_x, next_y = latitude(polygon[i+1]), longitude(polygon[i+1])
        area += x*next_y - next_x*y
        centroid_x += (x+next_x)* (x*next_y - next_x*y)
        centroid_y += (y+next_y)* (x*next_y - next_x*y)
...
def find_state_center(polygons):
    ...
    #Uses find_centroid to find the center of states represented as polygons
    state_x = 0
    state_y = 0
    total_area = 0

    for item in polygons:
        this_centroid = find_centroid(item)
        total_area += this_centroid[2]
        state_x += this_centroid[0]*this_centroid[2]
        state_y += this_centroid[1]*this_centroid[2]


    return make_position(state_x/total_area, state_y/total_area)
###################################
...
def group_tweets_by_state(tweets):
    ...
    """
    "*** YOUR CODE HERE ***"
    #runs a for loop and finds the minimum distance of all the states per tweet

    tweet_state = []
    for item in tweets:
        minimum_distance = geo_distance(tweet_location(item), find_state_center(us_states['CA']))
        min_key = 'CA'
        for state in us_states:
            if  minimum_distance > geo_distance(tweet_location(item), find_state_center(us_states[state])):
                min_key = state
                minimum_distance = geo_distance(tweet_location(item), find_state_center(us_states[state]))
        tweet_state.append([min_key, item])
    return group_by_key(tweet_state)       
...
def average_sentiments(tweets_by_state):
    ...
    def zero_checker(list):
        if len(list) == 0:
            del list
    sentiment_dict = {}
    for state in tweets_by_state:
        sentiment_sum, counter = 0,0
        zero_checker(tweets_by_state[state])
        for tweet in tweets_by_state[state]:
            if has_sentiment(analyze_tweet_sentiment(tweet)):
                ...
                counter += 1
                sentiment_dict[state] = sentiment_sum/counter
