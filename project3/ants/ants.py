...
class Place:
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            elif insect.can_contain(self.ant) == True and self.ant.container == False:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if hasattr(insect, 'name') and insect.name == 'Queen' and insect.number == 0:
                return
...
class ThrowerAnt(Ant):
    ...
    damage = 1
    food_cost = 4
    min_range = 0 #Hard Variables for number of bees to be passed through
    max_range = 10
    ...
    def nearest_bee(self, hive):
        ...
        """
        "*** YOUR CODE HERE ***"
        loc = self.place #Variable Set Location
        setter = 0 #Parses through variable
        while loc.entrance != None or loc != hive: #goes through bees available
            if len(loc.bees)> 0 and setter >= self.min_range and setter <= self.max_range:
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee): #consumes beee
        "***"
        if bee != None:
...
class QueenAnt (ScubaThrower):  # You should change this line
    ...
    def __init__(self, armor=1):
        "*** YOUR CODE HERE ***"
        Insect.__init__(self, armor)
        self.number = QueenAnt.queen_count
        QueenAnt.queen_count = QueenAnt.queen_count + 1
    ...
    def action(self, colony):
        ...
        """
        "*** YOUR CODE HERE ***"
        def traverse_tunnel(moving_forward):
            this_location = self.place
            while this_location != None:
                ant_to_buff = this_location.ant
                if this_location.ant != None and (this_location.ant not in QueenAnt.doubled_damage): #checks to make sure that there is an ant that has not been buffed
                    if ant_to_buff.name != 'Queen': #the queen can't buff herself
                        ant_to_buff.damage = ant_to_buff.damage*2
                        QueenAnt.doubled_damage.append(ant_to_buff)
                    if ant_to_buff.container == True and ant_to_buff.ant.name != 'Queen': #checks to see if the ant contains another
                        ant_to_buff = ant_to_buff.ant
                        ant_to_buff.damage = ant_to_buff.damage*2
                        QueenAnt.doubled_damage.append(ant_to_buff)

                if moving_forward:
                    this_location = this_location.exit #
                else:    
                    this_location = this_location.entrance
        ...
        traverse_tunnel(True) #traverses the tunnel backward, looking for ants to buff
        traverse_tunnel(False)  #traverse the tunnel forward, looking for ants to buff
...
def make_slow(action):
    ...
    def getName():
        return "Slow"
    def slow_down(bee, colony):
        if colony.time % 2 == 0:
            Bee.action(bee, colony)
    return slow_down, getName()
...
def apply_effect(effect, bee, duration):
    ...
    "*** YOUR CODE HERE ***"
    current, setName = effect(Bee.action)
    bee.effects[setName] = duration
    def implement_effect(colony):          
        nonlocal current, setName
        current(bee, colony)
        for impact in bee.effects:      
            if bee.effects[impact] >= 1:
                bee.effects[impact] -= 1
        if bee.effects["Stun"] == 0 and bee.effects["Slow"] == 0:
            bee.action = bee.default_action     
        elif bee.effects[setName] == 0:
            current, setName = flip(setName, bee)   

            
    bee.action = implement_effect
