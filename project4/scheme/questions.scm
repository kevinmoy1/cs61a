...
(define (list-partitions total max-pieces max-value)
  (define empty '())
  (define (count total max-pieces max-value parts)
    (cond
      ((= total 0) (list parts))
      ((or (= max-pieces 0) (< total 0)  (< max-value 1)) empty) 
      (else (append
              (count (- total max-value) (- max-pieces 1) max-value (append parts (list max-value)))
              (count total max-pieces (- max-value 1) parts)))))
    (count total max-pieces max-value empty)
)
  ...
  (cond ((atom? expr)
        ...
        ((or (lambda? expr)
         ...
         (let ((form   (car expr))
               ...
               (body   (cddr expr)))
           'YOUR-CODE-HERE
           (cons form (cons (apply-to-all analyze params) 
        ...
        ((let? expr)
         (let ((values (cadr expr))
               (body   (cddr expr)))
           'YOUR-CODE-HERE
           (define l12 (zip values))
           (define lst1 (car l12))
           (define lst2 (cadr l12))
