...
(list '(Ron Johnny) '(Ivy Caitlin Kenny Dylan) '(David Abdo Abe))
; expect ((Ron Johnny) (Ivy Caitlin Kenny Dylan) (David Abdo Abe))


(let ((Arani 7) (Dylan 7))
  (let ((csa (* Arani Dylan))) csa))
; expect 7

(define xray (lambda () 'Room111))
(xray)
; expect Room111
...
(define th_power (lambda (x) (* x x x)))
(th_power 5)
; expect 125
...
(+)
; expect Error
(define Dylan (print 6699))
; expect 2

(define point ( mu (l) (* m n)))
(define fat (lambda (m n) (point (/ m m))))
(fat 45 56)
; expect 56
...
(define (l? y) (or (and (not (z? (r y 100))) (z? (r y 4))) (z? (r y 400))))
